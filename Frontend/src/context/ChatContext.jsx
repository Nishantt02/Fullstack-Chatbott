...
//   const GEMINI_KEY = "AIzaSyDTJsJYlLrixwO_5syRboI0AXE6AXJ86rw";
...
export const ChatProvider = ({ children }) => {
  ...
  const fetchResponse = async () => {
    ...
    const now = Date.now();
    if (now - lastCallTime < 5000) return toast.error("Please wait a few seconds");

    if (isRequestInProgress) return toast.error("Response in progress...");
    ...
    try {
      ...
      // Save to backend
      await axios.post(`${server}/chat/add/${selected}`, { question: userPrompt, answer }, { headers: authHeader });
    } catch (error) {
      console.error(error);
      toast.error(error.response?.status === 429 ? "Rate limit reached" : "Something went wrong");
    } finally {
      isRequestInProgress = false;
      setNewRequestLoading(false);
    }
  ...
  const fetchChats = async () => {
    try {
      const { data } = await axios.get(`${server}/chat/all`, { headers: authHeader });
      const chatList = Array.isArray(data) ? data : Array.isArray(data.chats) ? data.chats : [];
      setChats(chatList);
      setSelected(chatList.length ? chatList[0]._id : null);
    } catch (error) {
      console.error(error);
      setChats([]);
    }
